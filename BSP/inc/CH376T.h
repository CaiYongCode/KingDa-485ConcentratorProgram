...
/* **************************************************************** ****************
 命令代码 (部分命令兼容CH375芯片, 但是输入数据或者输出数据的可能局部不同) 
     一个命令操作顺序包含:
 ...
 本文件默认会同时提供与CH375芯片命令码兼容的命令码格式（即去掉x和y之后), 如果不需要, 那
 么可以定义_NO_CH375_COMPATIBLE_禁止
*************************************************************************************/
//主要命令(手册一), 常用 
...
#define	CMD21_SET_BAUDRATE	0x02    /* 串口方式: 设置串口通讯波特率
                                           (上电或者复位后的默认波特率为9600bps,由D4/D5/D6引脚选择)
                                           输入: 波特率分频系数, 波特率分频常数 
                                           输出: 操作状态( CMD_RET_SUCCESS或CMD_RET_ABORT, 
                                           其它值说明操作未完成 ) */
...
#define	CMD20_CHK_SUSPEND	0x0B	/* 设备方式: 设置检查USB总线挂起状态的方式
                                           输入: 数据10H, 检查方式 
                                           00H=不检查USB挂起, 04H=以50mS为间隔检查USB挂起, 
                                           05H=以10mS为间隔检查USB挂起 */

#define	CMD20_SET_SDO_INT	0x0B    /* SPI接口方式: 设置SPI的SDO引脚的中断方式 
                                           输入: 数据16H, 中断方式 
                                           10H=禁止SDO引脚用于中断输出,在SCS片选无效时三态输出禁止,
...
#define	CMD11_SET_USB_MODE	0x15    /* 设置USB工作模式 */
                                        ...
                                        /*  00H=未启用的设备方式, 
                                          ...
                                          输出: 操作状态
                                            CMD_RET_SUCCESS或CMD_RET_ABORT,其它值说明操作未完成 */
...
#define	CMD01_RD_USB_DATA	0x28    /* 设备方式: 从当前USB中断的端点缓冲区读取数据块, 并释放缓冲区, 
                                                     相当于 CMD01_RD_USB_DATA0 + CMD00_UNLOCK_USB 
                                               输出: 长度, 数据流 */
...
#define	CMD0H_DISK_CONNECT	0x3     /* 主机文件模式/不支持SD卡: 检查磁盘是否连接 
                                           输出中断 */

#define	CMD0H_DISK_MOUNT	0x31    /* 主机文件模式: 初始化磁盘并测试磁盘是否就绪 
                                           输出中断*/ 

#define	CMD0H_FILE_OPEN		0x32	/* 主机文件模式: 打开文件或者目录(文件夹),
                                                         或者枚举文件和目录(文件夹) 
                                           输出中断 */
...
#define	CMD0H_FILE_ERASE	0x35	/* 主机文件模式: 删除文件,如果已经打开则直接删除,
                                                         否则对于文件会先打开再删除,子目录必须先打开
                                           输出中断 */

#define	CMD1H_FILE_CLOSE	0x36	/* 主机文件模式: 关闭当前已经打开的文件或者目录(文件夹) 
                                           输入: 是否允许更新文件长度  00H=禁止更新长度, 01H=允许更新长度 
                                           输出中断 */
...
#define	CMD10_SET_USB_SPEED	0x04	/* 设置USB总线速度, 在每次CMD11_SET_USB_MODE设置USB工作模式时
                                           会自动恢复到12Mbps全速 
                                           输入: 总线速度代码
...
#define	CMD01_TEST_CONNECT	0x16	/* 主机方式/不支持SD卡: 检查USB设备连接状态 
                                           输出: 状态( USB_INT_CONNECT或USB_INT_DISCONNECT
                                                 或USB_INT_USB_READY, 其它值说明操作未完成 ) */
...
#define	CMD10_SET_ENDP3		0x19	/* 设备方式(串口不支持): 设置USB端点0的发送器 
                                           输入:位7为1则位6为同步触发位, 否则同步触发位不变 
                                                位3~位0为事务响应方式: 0000~1000-就绪ACK, 
                                                                       1110-正忙NAK, 
                                                                       1111-错误STALL */

#define	CMD10_SET_ENDP4		0x1A	/* 设备方式(串口不支持): 设置USB端点1的接收器 
                                           输入:位7为1则位6为同步触发位,否则同步触发位不变 
                                                位3~位0为事务响应方式: 0000-就绪ACK, 
                                                                       1110-正忙NAK, 
                                                                       1111-错误STALL */
...
#define	CMD2H_ISSUE_TKN_X	0x4E	/* 主机方式: 发出同步令牌, 执行事务, 该命令可代替 
                                                     CMD10_SET_ENDP6/CMD10_SET_ENDP7 + CMD1H_ISSUE_TOKEN 
                                           输入:(同步标志, 事务属性) 
                                                同步标志的 位7为主机端点IN的同步触发位, 
                                                           位6为主机端点OUT的同步触发位, 
                                                           位5~位0必须为0 
                                                事务属性的 低4位是令牌, 
                                                           高4位是端点号
...
#define	SER_CMD_TIMEOUT		32	/* 串口命令超时时间, 单位为mS, 
                                           同步码之间及同步码与命令码之间的间隔应该尽量短, 
                                           超时后的处理方式为丢弃 */
...
/* 以下状态代码为特殊事件中断, 如果通过CMD20_CHK_SUSPEND启用USB总线挂起检查, 
那么必须处理USB总线挂起和睡眠唤醒的中断状态 */
...
/* 以下状态代码2XH-3XH用于USB主机方式的通讯失败代码 
   ...
   位5为1 
   位4指示当前接收的数据包是否同步 
   位3-位0指示导致通讯失败时USB设备的应答: 0010=ACK,   1010=NAK, 
                                           1110=STALL, 0011=DATA0, 
                                           1011=DATA1, XX00=超时 
...
typedef struct _BULK_ONLY_CBW {
	UINT32	CBW_Sig;
	UINT32	CBW_Tag;
	UINT8	CBW_DataLen0;	     /* 08H,输入: 数据传输长度,对于输入数据其有效值是0到48,
                                        对于输出数据其有效值为0到33 */
	UINT8	CBW_DataLen1;
	UINT16	CBW_DataLen2;
	UINT8	CBW_Flag;	     /* 0CH,输入: 传输方向等标志,位7为1则输入数据, 
                                        位为0则输出数据或者没有数据 */
	UINT8	CBW_LUN;
...
#define MAX_FILE_NAME_LEN  (13+1)   /* 文件名最大长度,最大长度是
                                      1个根目录符+8个主文件名+1个小数点+3个类型名+结束符=14 */
...
typedef union _CH376_CMD_DATA {
  ...
  struct {
    UINT8 mDirInfoIndex;	/* 输入参数: 指定需要读取的目录信息结构在扇区内的索引号, 
                                             0FFH则为当前已经打开的文件 */
  } DirInfoRead;		/* CMD1H_DIR_INFO_READ: 读取文件的目录信息 */
  
  union {
    UINT32 mByteOffset;	        /* 输入参数: 偏移字节数,以字节为单位的偏移量(总长度32位,低字节在前) */
    UINT32 mSectorLba;	        /* 返回: 当前文件指针对应的绝对线性扇区号,
                                        0FFFFFFFFH则已到文件尾(总长度32位,低字节在前) */
...
#define	VAR_UDISK_LUN		0x34	/* USB存储设备的逻辑单元号 
                                           位7~位4, USB存储设备的当前逻辑单元号,
                                                    CH376初始化USB存储设备后,
                                                    默认是访问0#逻辑单元 
                                           位3~位0, USB存储设备的最大逻辑单元号,
                                                    加1后等于逻辑单元数 */
#define	VAR_SEC_PER_CLUS	0x38	/* 逻辑盘的每簇扇区数 */
#define	VAR_FILE_DIR_INDEX	0x3B	/* 当前文件目录信息在扇区内的索引号 */
#define	VAR_CLUS_SEC_OFS	0x3C	/* 当前文件指针在簇内的扇区偏移,
                                           为0xFF则指向文件末尾,簇结束 */
/* 32位/4字节变量 */
#define	VAR_DISK_ROOT		0x44	/* 对于FAT16盘为根目录占用扇区数,
                                           对于FAT32盘为根目录起始簇号(总长度32位,低字节在前) */
