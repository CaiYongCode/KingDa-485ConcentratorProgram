...
{
  /* Set HSION bit */
  RCC->CR |= (u32)0x00000001;	  //选择内部8M时钟

  /* Reset SW[1:0], HPRE[3:0], PPRE1[2:0], PPRE2[2:0], ADCPRE[1:0] and MCO[2:0] bits */
  // MCO[2:0]       :没有时钟输出
  // ADCPRE[1:0]    :PCLK2 2分频后作为ADC时钟
  // PPRE2[2:0]		:HCLK不分频    控制高速APB2时钟
  // PPRE1[2:0]     :HCLK不分频    控制低速APB1时钟(PCLK1)的预分频系数	软件必须保证APB1时钟频率不超过36MHz。
  // HPRE[3:0]		:SYSCLK不分频  控制AHB时钟的预分频系数
  // SW[1:0]        :HSI作为系统时钟
  ...
  /* Reset HSEBYP bit */
  // HSEBYP  外部4-16MHz振荡器没有旁路	  只有在外部4-16MHz振荡器关闭的情况下，才能写入该位。
  RCC->CR &= (u32)0xFFFBFFFF;

  /* Reset PLLSRC, PLLXTPRE, PLLMUL[3:0] and USBPRE bits */
  //USBPRE        ：PLL时钟1.5倍分频作为USB时钟
  //PLLMUL[3:0]   ：PLL 2倍频输出
  //PLLXTPRE      ：HSE不分频  只能在关闭PLL时才能写入此位。
  //PLLSRC        ：HSI振荡器时钟经2分频后作为PLL输入时钟
...
void RCC_HSEConfig(u32 RCC_HSE)
{
  ...
  switch(RCC_HSE)
  {
    case RCC_HSE_ON:	  ////HSE振荡器开启。
      /* Set HSEON bit */
      RCC->CR |= CR_HSEON_Set;
      break;
      
    case RCC_HSE_Bypass:	  //外部4-16MHz外部晶体振荡器被旁路。
